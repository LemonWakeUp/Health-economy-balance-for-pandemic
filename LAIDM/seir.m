function [onset,R_hat,D,S,E,I,R]=seir(T,params,L,lockday,type)
population  = 9319700;
S           = zeros(1,length(T));
E           = zeros(1,length(T));
I           = zeros(1,length(T));
...
NTL_pro = sum(NTL_weight_WH.*type'.*xishu')./sum(NTL_weight_WH.*xishu');
...
for doy = start_date : end_date
    ...
    if doy>0
        ...
        I(doy) = I(doy-1) + symp*alpha1*E(doy-1) - gamma20*I(doy-1);
        D(doy) = gamma20*I(doy-1)*cfr;
        R(doy) = R(doy-1) + (1-symp)*gamma1*E(doy-1) + (gamma20*I(doy-1) - D(doy));
